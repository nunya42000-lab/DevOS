...
_detectOrphanIssues(srcCode) {
    ...
    scriptContent = scriptContent.replace(/\/\*[\s\S]*?\*\/|([^\\:]|^)\/\/.*$/gm, '$1');
    ...
    allElements.forEach(el => {
        ...
        const escapedId = id.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
        const wordRegex = new RegExp(`\\b${escapedId}\\b`);
        const cssRegex = new RegExp(`#${escapedId}(?![\\w-])`);

        const usedInScript = wordRegex.test(scriptContent);
        const usedInStyle = cssRegex.test(styleContent);
        const usedInHtml = htmlReferencedIds.has(id);

        if (!usedInScript && !usedInStyle && !usedInHtml) {
            // Find where this id is actually defined in the markup, to jump to it
