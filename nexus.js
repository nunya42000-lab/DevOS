...
const Nexus = {
    ...
    runIntel() {
        ...
        document.getElementById('intel-list').innerHTML = [...new Set(decls)].map(d => `<div class="list-entry">${d}</div>`).join('');
    ...
    copy() { navigator.clipboard.writeText(this.state.cm.state.sliceDoc(this.state.cm.state.selection.main.from, this.state.cm.state.selection.main.to) || this.state.cm.state.doc.toString()); this.log("Copied."); },
    cut() { 
        const s = this.state.cm.state.selection.main;
        if (s.from !== s.to) {
            navigator.clipboard.writeText(this.state.cm.state.sliceDoc(s.from, s.to));
    ...
    startVoice() {
        ...
        recognition.onresult = (event) => {
            ...
            document.getElementById('term-in').value = transcript;
...
// --- OPTIONAL ADVANCED MODULES (Paste at bottom of nexus.js) ---
const Hardware = {
    initShakeToUndo() {
        ...
        window.addEventListener('devicemotion', (e) => {
            ...
            if (delta > 15) { // Threshold
                ...
                // CM6 Undo Command
                if(window.CM6) window.CM6.undo({state: Nexus.state.cm.state, dispatch: Nexus.state.cm.dispatch});
