...
/**
 * CORE ARCHITECTURE: 
 * DevOS Nexus Prime is a modular PWA IDE. 
 * This file manages the Virtual File System (VFS), CodeMirror 6 (CM6) instances,
 * Gesture Engines, Peer-to-Peer Sync, and the Programmatic Scroll Fix.
 */

// --- 1. PROGRAMMATIC SCROLL ENGINE ---
// Explicitly controls CodeMirror internal scrolling to bypass browser touch bugs.
const ScrollEngine = {
    jump(lines) {
...
const NexusHistory = {
    ...
    restore(filename, timestamp) {
        ...
        if (snap) {
            ...
            Nexus.log(`Restored ${filename} to ${new Date(timestamp).toLocaleTimeString()}`, "var(--success)");
        }
    }
};
...
const Nexus = {
    ...
    maximizeEditor() {
        ...
        this.state.popupCm.dispatch({ effects: this.state.languageConf.reconfigure(langExt) });
    ...
    // --- SCROLL PILLAR INTERFACE ---
    initScrollPillar() {
        const pillar = document.createElement('div');
        pillar.className = 'scroll-pillar';
        const cfg = [
            { l: 'TOP', v: 'TOP' }, { l: '▲100', v: -100 }, { l: '▲25', v: -25 }, { l: '▲5', v: -5 },
            { l: '▼5', v: 5 }, { l: '▼25', v: 25 }, { l: '▼100', v: 100 }, { l: 'END', v: 'BOTTOM' }
        ];
        cfg.forEach(i => {
    ...
    type(val, fromSync = false) {
        ...
        if (val === 'BACKSPACE') {
            if (cursor > 0) view.dispatch({ changes: { from: cursor - 1, to: cursor, insert: "" } });
        } else {
            view.dispatch({
        ...
        const helpHTML = `
            <div style="line-height: 1.6;">
                ...
                <ul style="padding-left: 20px;">
                    <li><b>File Importer:</b> Use the 'Import' button to load multiple files or full .zip projects.</li>
                    <li><b>Voice Commands (🎙️):</b> Click the mic and dictate commands like "build", "nuke", or "pwa".</li>
                    <li><b>Math Engine:</b> In the terminal, type <code>= 50 * 4</code> to compute equations instantly.</li>
                    <li><b>Gestures:</b> Pinch with two fingers to zoom the UI.</li>
                    <li><b>Scroll Pillar:</b> Use the bar on the right to jump lines programmatically.</li>
                </ul>
            </div>`;
        this.showModal("📖 Help & Documentation", helpHTML);
    },

    // --- FILE PORTER (ZIP & MULTI-FILE) ---
    initImporter() {
    ...
    formatCode() {
        const view = this.state.popupCm || this.state.cm;
        if (!this.state.active || !view) return;
        ...
        try {
            ...
            view.dispatch({ changes: { from: 0, to: code.length, insert: formatted } });
    ...
    copy() { 
        const view = this.state.popupCm || this.state.cm;
        if (!view) return;
        navigator.clipboard.writeText(view.state.sliceDoc(view.state.selection.main.from, view.state.selection.main.to) || view.state.doc.toString()); 
        this.log("Copied."); 
    },
    cut() { 
        const view = this.state.popupCm || this.state.cm;
        ...
        if (s.from !== s.to) {
            navigator.clipboard.writeText(view.state.sliceDoc(s.from, s.to));
    ...
    initShakeToUndo() {
        ...
        window.addEventListener('devicemotion', (e) => {
            ...
            if (delta > 20) { 
                const view = Nexus.state.popupCm || Nexus.state.cm;
                if(window.CM6 && view) {
                    Nexus.log("Shake: Undoing last action...");
